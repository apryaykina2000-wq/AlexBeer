...
class GameScene extends Phaser.Scene {
  ...
  create(){
    ...
    // земля
    this.ground = this.add.tileSprite(0, HEIGHT-220, WIDTH, 220, 'ground_tile').setOrigin(0);
    ...
    this.groundY = HEIGHT-220+54; // линия травы чуть выше для травы
    // игрок
    ...
    this.input.on('pointerup', ()=> {}); // iOS wake up
  ...
  spawnObstacle(){
    const o = this.obstacles.create(WIDTH+60, this.groundY, 'obstacle_block').setOrigin(0.5,1).setScale(0.9);
...
class ResultScene extends Phaser.Scene {
  ...
  create(){
    ...
    // Кнопки
    const btnAgain = this.add.image(WIDTH/2, HEIGHT-180, 'btn_large').setInteractive({useHandCursor:true});
    ...
    const btnMenu = this.add.image(WIDTH/2, HEIGHT-90, 'btn_large').setScale(0.7).setInteractive({useHandCursor:true});
