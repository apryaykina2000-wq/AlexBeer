...
class MenuScene extends Phaser.Scene {
  ...
  create(){
    ...
    // Заголовок
    const title = this.add.text(WIDTH/2, 90, 'ALEXBEER', {
    ...
    const bestText = this.add.text(WIDTH/2, 170, 'РЕКОРД: ' + best, {
      fontFamily: 'GameFont, Arial',
      fontSize: '28px',
      color: '#ffffff',
      stroke: '#0d3b3b',
    ...
    // Витрина мини-скрин
    const preview = this.add.container(80,220).setDepth(0);
    ...
    // Кнопка Play
    const btn = this.add.image(WIDTH/2, HEIGHT-100, 'btn_large').setInteractive({ useHandCursor: true });
...
class GameScene extends Phaser.Scene {
  ...
  create(){
    ...
    // земля
    this.ground = this.add.tileSprite(0, HEIGHT-120, WIDTH, 120, 'ground_tile').setOrigin(0);
    // физическая линия земли
    this.physics.world.setBounds(0,0,WIDTH,HEIGHT);
    this.groundY = HEIGHT-120+40; // линия травы
    // игрок
